...
class ViT_dualModal_d(nn.Module):
    ...
    def forward(self, img_d):
        ...
        # x = self.to_patch_embedding_fMRI(img)
        # b, n, _ = x.shape
        # x += self.pos_embedding_fMRI[:, :(n)]
        # x = self.dropout(x)
        # # print(x_f.shape)
        # x = self.transformer_fMRI(x)
        # # print(x_f.shape)
        # x = self.inverse_to_patch_embedding_fMRI(x)
