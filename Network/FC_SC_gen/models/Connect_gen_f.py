...
class MoELayer(nn.Module):
    ...
    def __init__(self, d_model, d_ff, num_experts=200, k=200):
        ...
        self.gate = nn.Linear(d_model, 10)
    def forward(self, x):
...
class Transformer_encoder(nn.Module):
    def  __init__(self, opt, dim=256):
        ...
        self.ViT_CEB = ViT(image_height=17, patch_size=opt.patch_size_refine, dim=opt.dim_refine_f, depth=opt.depth_refine_f,
                       heads=opt.heads, mlp_dim=opt.mlp_dim_refine_f)
        self.ViT_CON = ViT(image_height=32, patch_size=opt.patch_size_refine, dim=opt.dim_refine_f, depth=opt.depth_refine_f,
                       heads=opt.heads, mlp_dim=opt.mlp_dim_refine_f)
        self.ViT_DMN = ViT(image_height=34, patch_size=opt.patch_size_refine, dim=opt.dim_refine_f, depth=opt.depth_refine_f,
                       heads=opt.heads, mlp_dim=opt.mlp_dim_refine_f)
        self.ViT_OCC = ViT(image_height=21, patch_size=opt.patch_size_refine, dim=opt.dim_refine_f, depth=opt.depth_refine_f,
                       heads=opt.heads, mlp_dim=opt.mlp_dim_refine_f)
        self.ViT_FPN = ViT(image_height=22, patch_size=opt.patch_size_refine, dim=opt.dim_refine_f, depth=opt.depth_refine_f,
                       heads=opt.heads, mlp_dim=opt.mlp_dim_refine_f)
        self.ViT_SMA = ViT(image_height=34, patch_size=opt.patch_size_refine, dim=opt.dim_refine_f, depth=opt.depth_refine_f,
                       heads=opt.heads, mlp_dim=opt.mlp_dim_refine_f)
...
class Transformer_decoder(nn.Module):
    def  __init__(self, opt):
        ...
        self.opt = opt
        # self.codebook_size = codebook_size
        # self.ViT = ViT(image_size=160, patch_size=opt.patch_size_refine, num_classes=opt.n_classes, dim=opt.dim_refine depth=opt.depth_refine,
        #                heads=opt.heads, mlp_dim=opt.mlp_dim_refine)
        self.ViT_CEB = ViT(image_height=17, patch_size=opt.patch_size_refine, dim=opt.dim_refine_f,
                           depth=opt.depth_refine_f,
                           heads=opt.heads, mlp_dim=opt.mlp_dim_refine_f)
        self.ViT_CON = ViT(image_height=32, patch_size=opt.patch_size_refine, dim=opt.dim_refine_f,
                           depth=opt.depth_refine_f,
                           heads=opt.heads, mlp_dim=opt.mlp_dim_refine_f)
        self.ViT_DMN = ViT(image_height=34, patch_size=opt.patch_size_refine, dim=opt.dim_refine_f,
                           depth=opt.depth_refine_f,
                           heads=opt.heads, mlp_dim=opt.mlp_dim_refine_f)
        self.ViT_OCC = ViT(image_height=21, patch_size=opt.patch_size_refine, dim=opt.dim_refine_f,
                           depth=opt.depth_refine_f,
                           heads=opt.heads, mlp_dim=opt.mlp_dim_refine_f)
        self.ViT_FPN = ViT(image_height=22, patch_size=opt.patch_size_refine, dim=opt.dim_refine_f,
                           depth=opt.depth_refine_f,
                           heads=opt.heads, mlp_dim=opt.mlp_dim_refine_f)
        self.ViT_SMA = ViT(image_height=34, patch_size=opt.patch_size_refine, dim=opt.dim_refine_f,
                           depth=opt.depth_refine_f,
                           heads=opt.heads, mlp_dim=opt.mlp_dim_refine_f)
        self.criterion = torch.nn.CrossEntropyLoss()
        self.apply_grad_penalty_every =4
...
class Transformer_dirsc(nn.Module):
    def  __init__(self, opt):
        ...
        #                heads=opt.heads, mlp_dim=opt.mlp_dim_refine)
        self.ViT_CEB = ViT_discr(image_height=17, patch_size=opt.patch_size_refine, dim=opt.dim_refine_f,
                           depth=opt.depth_refine_f,
                           heads=opt.heads, mlp_dim=opt.mlp_dim_refine_f)
        self.ViT_CON = ViT_discr(image_height=32, patch_size=opt.patch_size_refine, dim=opt.dim_refine_f,
                           depth=opt.depth_refine_f,
                           heads=opt.heads, mlp_dim=opt.mlp_dim_refine_f)
        self.ViT_DMN = ViT_discr(image_height=34, patch_size=opt.patch_size_refine, dim=opt.dim_refine_f,
                           depth=opt.depth_refine_f,
                           heads=opt.heads, mlp_dim=opt.mlp_dim_refine_f)
        self.ViT_OCC = ViT_discr(image_height=21, patch_size=opt.patch_size_refine, dim=opt.dim_refine_f,
                           depth=opt.depth_refine_f,
                           heads=opt.heads, mlp_dim=opt.mlp_dim_refine_f)
        self.ViT_FPN = ViT_discr(image_height=22, patch_size=opt.patch_size_refine, dim=opt.dim_refine_f,
                           depth=opt.depth_refine_f,
                           heads=opt.heads, mlp_dim=opt.mlp_dim_refine_f)
        self.ViT_SMA = ViT_discr(image_height=34, patch_size=opt.patch_size_refine, dim=opt.dim_refine_f,
                           depth=opt.depth_refine_f,
                           heads=opt.heads, mlp_dim=opt.mlp_dim_refine_f)
        self.criterion = torch.nn.CrossEntropyLoss()
        self.apply_grad_penalty_every =4
...
class generator(nn.Module):
    ...
    def forward(self, individual, group):
        individual_en = self.Transformer_Individual_encode(individual)
        individual_out = self.Transformer_Individual_decode(individual_en)
        group_en = self.Transformer_encode(group)
...
class discriminator(nn.Module):
    ...
    def forward(self, arr):
        en_fc = self.Transformer_encode(arr)
        out = self.Transformer_decode(en_fc)

        return out

class Connect_gen_discr_f(nn.Module):
    def __init__(self, *, opt):
        super().__init__()
