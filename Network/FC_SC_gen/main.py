...
def run(fold_id, opt):
    ...
    if opt.refine == True:
        ...
        with torch.no_grad():
            checkpoint = torch.load(opt.pretrain_foundation_path_f, map_location='cuda:0')['state_dict']
        ...
        with torch.no_grad():
            checkpoint = torch.load(opt.pretrain_foundation_path_d, map_location='cuda:0')['state_dict']
    ...
    for i in range(1, opt.n_epochs+1):
        ...
        if not opt.no_train and opt.refine == False:
            ...
            train_epoch_refine(i, fold_id, train_loader, model, opt,
                        train_logger, train_batch_logger, writer, optimizer, atlas_fMRI, atlas_dti, atlas_f_mask, atlas_d_mask,atlas_f_eq,atlas_d_eq, model_pretrain_f,model_pretrain_d)
       # if not opt.no_val:
