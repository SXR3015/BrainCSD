...
def train_epoch(epoch, fold_id, data_loader, model,
    ...
    for i ,(inputs) in enumerate(data_loader):
        ...
        if opt.mode_net == 'Foundation' :
                   re_fMRI = model(target_fMRI.unsqueeze(1).cuda())
                   ...
                   loss =(F.mse_loss(re_fMRI,(target_fMRI.unsqueeze(1)*atlas_fMRI.unsqueeze(0)).cuda()) +
                            bce_discr_loss(re_fMRI,(target_fMRI.unsqueeze(1)*atlas_fMRI.unsqueeze(0)).cuda()))
        ...
        if i in index_train and epoch % 1 == 0:
                    index = np.random.randint(0, re_fMRI.shape[0], size=1)
                    save_path = opt.root_path +'/Synthesis/'+opt.structure
                    if not os.path.exists(save_path):
                        os.makedirs(save_path)
                    gen_path = save_path + '/train_fmri_epoch%d_iter%d.nii' % (
                    epoch, i)
                    # print(affine_fMRI[index].shape)
                    nib.Nifti1Image(re_fMRI[index].detach().squeeze().cpu().numpy() ,
                                    affine_fMRI[index].detach().squeeze().cpu().numpy()).to_filename(gen_path)
                    ...
                    tar_path = save_path + '/train_fmri_tar_epoch%d_iter%d.nii' % (epoch, i)
                    nib.Nifti1Image((target_fMRI.unsqueeze(1)*atlas_fMRI.unsqueeze(0))[index].squeeze().cpu().numpy(),
                        affine_fMRI[index].detach().squeeze().cpu().numpy()).to_filename(tar_path)
                    ...
                    source_path = save_path + '/train_fmri_source_epoch%d_iter%d.nii' % (epoch, i)
                    nib.Nifti1Image((target_fMRI.unsqueeze(1))[index].squeeze().cpu().numpy(),
                                    affine_fMRI[index].detach().squeeze().cpu().numpy()).to_filename(source_path)
