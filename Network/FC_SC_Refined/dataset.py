...
def mat_loader(path):
    ...
    if 'FC' in path:
        try:
           ...
           group_data = sio.loadmat('/data1/sxr/Unified_model/code/FC_SC_Refined/important_data/group_self.mat')['fc']
        except:
            mat_data = sio.loadmat(path)['z']
            group_data = sio.loadmat('/data1/sxr/Unified_model/code/FC_SC_Refined/important_data/group_fcon.mat')['fc']
    else:
        mat_data = sio.loadmat(path)['SC']
    # img_arr = np.array(img_pil.get_fdata()) change the function get_data() to get_fdata()
    img_arr_cleaned = np.nan_to_num(mat_data,nan=0,posinf=1,neginf=-1)  # Replace NaN with zero and infinity with large finite numbers.
    # img_arr_cleaned[np.isinf(img_arr_cleaned] = 1
    max_ = np.max(np.max(img_arr_cleaned))
    min_ = np.min(np.min(img_arr_cleaned))
    range_ = max_ - min_
    if range_ > 0:
    ...
    img_arr_cleaned_group = np.nan_to_num(group_data , nan=0, posinf=1, neginf=-1)
    max_group = np.max(np.max(img_arr_cleaned_group ))
    min_group  = np.min(np.min(img_arr_cleaned_group ))
    range_group  = max_group - min_group
    if range_group  > 0:
        img_arr_cleaned_group  = (img_arr_cleaned_group -min_group) /range_group
    img_pil_group = torch.from_numpy(img_arr_cleaned_group)
    return img_pil,img_pil_group
...
class TrainSet(Dataset):
    ...
    def __init__(self, fold_id, nii_loader=nii_loader):
        ...
        with open(csv_dir + '/train_fold%s.csv' % str(fold_id), 'r') as csvfile:
            reader = csv.reader(csvfile)
            file_connect= [row[2] for row in reader]
        self.image = np.array([np.flip(file_train_fmri), np.flip(file_train_diffusion), np.flip(file_connect)])
        # if opt.refine == True:
        ...
        for file in fn:
            if "diffusion" in file:
              ...
              img_arr.append(fmri_affine)
                ...
                Connect, group = self.mat_loader(file)
                img_arr.append(Connect)
                img_arr.append(group)
        return img_arr
