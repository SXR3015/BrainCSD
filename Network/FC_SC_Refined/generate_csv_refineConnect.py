...
imaging_path = r'/data1/sxr/Unified_model/data/Diffusion'
...
if 'FC' in opt.Connect:
    ...
    for sub in Subs_had_FC:
        ...
        if os.path.isfile(sub_rsfmri) and os.path.isfile(subs_dff):
            subs_diff.append(subs_dff)
            subs_rsfmri.append(sub_rsfmri)
            Subs_connect.append(sub)
        # Subs_connect.remove(sub)
...
# print((np.array(subs_rsfmri).shape))
# print((np.array(subs_diff).shape))
# print(np.array(Subs_connect).shape)
# if csv_contain == True:
#     fea_num = fea_num-1
