...
class ViT(nn.Module):
    def __init__(self, *, image_height, patch_size, num_classes, dim, depth, heads, mlp_dim, pool='cls', channels=1,
...
class ViT_discr(nn.Module):
    def __init__(self, *, image_height, patch_size, num_classes, dim, depth, heads, mlp_dim, pool = 'cls', channels = 1, image_width=160, dim_head = 64, dropout = 0., emb_dropout = 0.):
        super().__init__()
        # image_height, image_width = pair(image_size)
        # patch_height, patch_width = pair(patch_size)
        patch_height = image_height
...
class Transformer_encoder(nn.Module):
    def  __init__(self, opt, dim=256):
        ...
        self.ViT_CEB = ViT(image_height=17, patch_size=opt.patch_size, num_classes=opt.n_classes, dim=opt.dim, depth=opt.depth,
                       heads=opt.heads, mlp_dim=opt.mlp_dim)
        self.ViT_CON = ViT(image_height=32, patch_size=opt.patch_size, num_classes=opt.n_classes, dim=opt.dim, depth=opt.depth,
                       heads=opt.heads, mlp_dim=opt.mlp_dim)
        self.ViT_DMN = ViT(image_height=34, patch_size=opt.patch_size, num_classes=opt.n_classes, dim=opt.dim, depth=opt.depth,
                       heads=opt.heads, mlp_dim=opt.mlp_dim)
        self.ViT_OCC = ViT(image_height=21, patch_size=opt.patch_size, num_classes=opt.n_classes, dim=opt.dim, depth=opt.depth,
                       heads=opt.heads, mlp_dim=opt.mlp_dim)
        self.ViT_FPN = ViT(image_height=22, patch_size=opt.patch_size, num_classes=opt.n_classes, dim=opt.dim, depth=opt.depth,
                       heads=opt.heads, mlp_dim=opt.mlp_dim)
        self.ViT_SMA = ViT(image_height=34, patch_size=opt.patch_size, num_classes=opt.n_classes, dim=opt.dim, depth=opt.depth,
                       heads=opt.heads, mlp_dim=opt.mlp_dim)
...
class Transformer_decoder(nn.Module):
    def  __init__(self, opt):
        ...
        #                heads=opt.heads, mlp_dim=opt.mlp_dim)
        self.ViT_CEB = ViT(image_height=17, patch_size=opt.patch_size, num_classes=opt.n_classes, dim=opt.dim,
                           depth=opt.depth,
                           heads=opt.heads, mlp_dim=opt.mlp_dim)
        self.ViT_CON = ViT(image_height=32, patch_size=opt.patch_size, num_classes=opt.n_classes, dim=opt.dim,
                           depth=opt.depth,
                           heads=opt.heads, mlp_dim=opt.mlp_dim)
        self.ViT_DMN = ViT(image_height=34, patch_size=opt.patch_size, num_classes=opt.n_classes, dim=opt.dim,
                           depth=opt.depth,
                           heads=opt.heads, mlp_dim=opt.mlp_dim)
        self.ViT_OCC = ViT(image_height=21, patch_size=opt.patch_size, num_classes=opt.n_classes, dim=opt.dim,
                           depth=opt.depth,
                           heads=opt.heads, mlp_dim=opt.mlp_dim)
        self.ViT_FPN = ViT(image_height=22, patch_size=opt.patch_size, num_classes=opt.n_classes, dim=opt.dim,
                           depth=opt.depth,
                           heads=opt.heads, mlp_dim=opt.mlp_dim)
        self.ViT_SMA = ViT(image_height=34, patch_size=opt.patch_size, num_classes=opt.n_classes, dim=opt.dim,
                           depth=opt.depth,
                           heads=opt.heads, mlp_dim=opt.mlp_dim)
        self.criterion = torch.nn.CrossEntropyLoss()
        self.apply_grad_penalty_every =4
...
class Transformer_dirsc(nn.Module):
    def  __init__(self, opt):
        ...
        #                heads=opt.heads, mlp_dim=opt.mlp_dim)
        self.ViT_CEB = ViT_discr(image_height=17, patch_size=opt.patch_size, num_classes=opt.n_classes, dim=opt.dim,
                           depth=opt.depth,
                           heads=opt.heads, mlp_dim=opt.mlp_dim)
        self.ViT_CON = ViT_discr(image_height=32, patch_size=opt.patch_size, num_classes=opt.n_classes, dim=opt.dim,
                           depth=opt.depth,
                           heads=opt.heads, mlp_dim=opt.mlp_dim)
        self.ViT_DMN = ViT_discr(image_height=34, patch_size=opt.patch_size, num_classes=opt.n_classes, dim=opt.dim,
                           depth=opt.depth,
                           heads=opt.heads, mlp_dim=opt.mlp_dim)
        self.ViT_OCC = ViT_discr(image_height=21, patch_size=opt.patch_size, num_classes=opt.n_classes, dim=opt.dim,
                           depth=opt.depth,
                           heads=opt.heads, mlp_dim=opt.mlp_dim)
        self.ViT_FPN = ViT_discr(image_height=22, patch_size=opt.patch_size, num_classes=opt.n_classes, dim=opt.dim,
                           depth=opt.depth,
                           heads=opt.heads, mlp_dim=opt.mlp_dim)
        self.ViT_SMA = ViT_discr(image_height=34, patch_size=opt.patch_size, num_classes=opt.n_classes, dim=opt.dim,
                           depth=opt.depth,
                           heads=opt.heads, mlp_dim=opt.mlp_dim)
        self.ViT_CEB_label = ViT_discr(image_height=17, patch_size=opt.patch_size, num_classes=opt.n_classes, dim=opt.dim,
                           depth=opt.depth,
                           heads=opt.heads, mlp_dim=opt.mlp_dim)
        self.ViT_CON_label = ViT_discr(image_height=32, patch_size=opt.patch_size, num_classes=opt.n_classes, dim=opt.dim,
                           depth=opt.depth,
                           heads=opt.heads, mlp_dim=opt.mlp_dim)
        self.ViT_DMN_label = ViT_discr(image_height=34, patch_size=opt.patch_size, num_classes=opt.n_classes, dim=opt.dim,
                           depth=opt.depth,
                           heads=opt.heads, mlp_dim=opt.mlp_dim)
        self.ViT_OCC_label = ViT_discr(image_height=21, patch_size=opt.patch_size, num_classes=opt.n_classes, dim=opt.dim,
                           depth=opt.depth,
                           heads=opt.heads, mlp_dim=opt.mlp_dim)
        self.ViT_FPN_label = ViT_discr(image_height=22, patch_size=opt.patch_size, num_classes=opt.n_classes, dim=opt.dim,
                           depth=opt.depth,
                           heads=opt.heads, mlp_dim=opt.mlp_dim)
        self.ViT_SMA_label = ViT_discr(image_height=34, patch_size=opt.patch_size, num_classes=opt.n_classes, dim=opt.dim,
                           depth=opt.depth,
                           heads=opt.heads, mlp_dim=opt.mlp_dim)
        self.criterion = torch.nn.CrossEntropyLoss()
        self.apply_grad_penalty_every =4
    ...
    def forward(self, x):
        try :
            ...
            x_discr_CEB_source = self.ViT_CEB_label(x[0][:, :, 0:17, ...])
            x_discr_CON_source = self.ViT_CON_label(x[0][:, :, 17:49, ...])
            x_discr_DMN_source = self.ViT_DMN_label(x[0][:, :, 49:83, ...])
            x_discr_OCC_source = self.ViT_OCC_label(x[0][:, :, 83:104, ...])
            x_discr_FPN_source = self.ViT_FPN_label(x[0][:, :, 104:126, ...])
            x_discr_SMA_source = self.ViT_SMA_label(x[0][:, :, 126:, ...])
            x_source = torch.concatenate(
                (x_discr_CEB_source, x_discr_CON_source, x_discr_DMN_source, x_discr_OCC_source, x_discr_FPN_source, x_discr_SMA_source), dim=2)
            x_discr_CEB_target = self.ViT_CEB_label(x[0][:, :, 0:17, ...])
            x_discr_CON_target = self.ViT_CON_label(x[0][:, :, 17:49, ...])
            x_discr_DMN_target = self.ViT_DMN_label(x[0][:, :, 49:83, ...])
            x_discr_OCC_target = self.ViT_OCC_label(x[0][:, :, 83:104, ...])
            x_discr_FPN_target = self.ViT_FPN_label(x[0][:, :, 104:126, ...])
            x_discr_SMA_target = self.ViT_SMA_label(x[0][:, :, 126:, ...])
            x_target = torch.concatenate(
                (x_discr_CEB_target, x_discr_CON_target, x_discr_DMN_target, x_discr_OCC_target, x_discr_FPN_target, x_discr_SMA_target), dim=2)
