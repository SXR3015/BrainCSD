...
class MoELayer_time(nn.Module):
    ...
    def forward(self, x):
        ...
        gate_logits_check = self.gate(flat_x)  # [batch*seq, num_experts]
        gates_times = self.gate_time(flat_x)
        # Convert to probabilities via softmax
        gate_probs_check = F.softmax(gate_logits_check, dim=-1)  # [batch*seq, num_experts]
        gate_probs_times = F.softmax(gate_logits_check, dim=-1)
        # Select top-k experts for each token
        topk_probs_check, topk_indices_check = torch.topk(gate_probs_check, 2, dim=-1)  # both [batch*seq, k]
        topk_probs_times, topk_indices_times= torch.topk(gate_probs_times, 200, dim=-1)  # both [batch*seq, k]
...
class Conn_gen(nn.Module):
    def __init__(self, *, dim, depth, heads, mlp_dim, channels=1,
        ...
        num_patches_fMRI = ((image_height_fMRI // patch_size)
                            * (image_width_fMRI // patch_size)) * (image_depth_fMRI // patch_size)
    ...
    def forward(self, img_f, img_d, atlas_f, atlas_d):
        ...
        x_f = self.to_patch_embedding(img_f)
        b_f, n_f, _ = x_f.shape
        x_f += self.pos_embedding[:, :(n_f)]
        x_f = self.dropout(x_f)

        x_f = self.transformer(x_f)
        x_f = self.inverse_to_patch_embedding(x_f)
        f_flatten = x_d.view(b_f, n_f, -1)
