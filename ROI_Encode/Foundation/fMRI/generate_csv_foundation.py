...
csv_save_dir = OsJoin('csv/', opt.mode_net)
test_ratio = 0.0001
n_fold = 1000
...
for filename in os.listdir(least_subs_name):
    ...
    #   sub = filename.split('zfc_Covswra_')[1].split('_rsfMRI_timeseries_Dosenbach164.mat')[0]
      ...
      if 'Hospital' in filename:
          continue
      filename_path = os.path.join(opt.data_root_path,'Diffusion', filename)
      Subs.append(filename_path)
...
subs_rsfmri = list()
for sub in Subs:
    sub_rsfmri = sub.replace('_diffusion.nii', '_rsfMRI.nii')
    sub_rsfmri = sub_rsfmri.replace('Diffusion', 'Function')
...
if len(Subs) > 0:
    ...
    data_list = np.vstack((np.array(subs_rsfmri), np.array(Subs))).T
...
kf = KFold(n_splits=1000, shuffle=False)
n = 0
